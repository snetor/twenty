...
Ce dépôt est le fork Snetor de Twenty. Il porte du code métier — cloisonnement des données par
portée commerciale, dérivation automatique du pays, périmètre des surfaces en contexte système.

**Avant toute montée de version amont, et avant tout patch dans du code amont, lire
[`docs/snetor/FORK.md`](docs/snetor/FORK.md).** Il porte l'inventaire des modifications, leurs
points d'ancrage exacts, les specs-sentinelles qui les défendent, et la checklist de montée.

Deux règles qui ont chacune coûté quelque chose :

- Un patch s'accroche à une **interface publique** (décorateur, module NestJS, listener, méthode
  publique), jamais au milieu du corps d'une méthode privée — trois ancrages sur cinq ont disparu
  en neuf releases, sans erreur de compilation.
- Une pull request qui fusionne l'amont se merge en **merge commit**, jamais en squash : un squash
  efface la généalogie et le merge suivant recalcule des mois d'écart.

Déploiement : image construite par `.github/workflows/snetor-deploy.yaml` vers l'ACR privé, puis
GitOps dans `snetor/azure-landing-zone`.

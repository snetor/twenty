...
export class WorkspaceRepository<TEntity extends ObjectLiteral = ObjectRecord> {
  ...
  async runMutation({
    ...
    if (!rowLevelPermissionsApplied) {
      ...
      // vrai, l'appelant a déjà déclenché `onBeforeExecute`, qui a posé notre filtre.
      //
      // 🔴 Un UPDATE n'a pas de SELECT préalable. Sans ce prédicat, un utilisateur qui
      // connaît un `id` hors de son périmètre écrit dessus — c'est arrivé jusqu'au
      // 2026-08-30.
      this.applyCountryPermissionFilterPredicate(selectQueryBuilder);
    }
  ...
  private onBeforeExecute(queryBuilder: WorkspaceSelectQueryBuilder): void {
    this.applyRowLevelPermissionPredicates(queryBuilder);
    // Snetor — cloisonnement par portefeuille, point d'application unique en LECTURE.
    // `createQueryBuilder()` injecte ce hook dans tout builder de lecture, donc un seul
  ...
  // Snetor — voir `apply-country-permission-filter.util.ts`. Volontairement séparé de
  // `applyRowLevelPermissionPredicates` : ce dernier est gouverné par
  // `shouldBypassPermissionChecks`, alors que notre filtre décide lui-même de s'abstenir
  // (contexte non-utilisateur : clé API d'ingestion, contexte système).
  private applyCountryPermissionFilterPredicate(
    queryBuilder: WorkspaceSelectQueryBuilder,
  ): void {
